...
public:
  mmu_t(simif_t* sim, endianness_t endianness, processor_t* proc, cache_t* cache);
  ~mmu_t();

  template<typename T>
  T ALWAYS_INLINE load(reg_t addr, xlate_flags_t xlate_flags = {}) {
    std::cout<<"mmu load is called\n";
    target_endian<T> res;
    auto access_info = generate_access_info(addr, LOAD, xlate_flags);
    reg_t paddr = translate(access_info, sizeof(T)); 
    std::cout<<"virtual adress from load is: "<<addr<<"\nphysical adress from load is: "<<paddr<<"\n";
    if (cache->get_name() == "regular cache" && cache->is_active())
    {
     std::cout<<"load_to_proc is called\n";
     if(cache->get_name() == "regular cache" && cache->load_to_proc((uint8_t*)&res, paddr, sizeof(T)) == true)
     {
      std::cout<<"load_to_proc is succeed\n";
    ...
    if (likely(!xlate_flags.is_special_access() && aligned && tlb_hit)) {
      ...
      if(cache->get_name() == "regular cache" && cache->is_active())
      {
       std::cout<<"load_to_cache tlb is called\n";
       cache->load_to_cache(paddr, (uint8_t*) (tlb_data[vpn % TLB_ENTRIES].host_offset + addr));//!!!!
       bool is_prefetched = cache->prefetch_on_miss(paddr, (uint8_t*) (tlb_data[vpn % TLB_ENTRIES].host_offset + addr));
      }
    } else {
       load_slow_path(addr, sizeof(T), (uint8_t*)&res, xlate_flags);
       std::cout<<"load_to_cache from load_slow_path is called\n";
      }

    if (unlikely(proc && proc->get_log_commits_enabled()))
      proc->state.log_mem_read.push_back(std::make_tuple(addr, 0, sizeof(T)));
    /*std::cout<<"data at host adress from mmu.h load is: ";
    for (int i = 0;i<64;i++)
    {
     std::cout<<(reg_t) *((uint8_t*)&res + i)<<" ";
    }
    std::cout<<"\n";*/
    return from_target(res);
  }
  ...
  void ALWAYS_INLINE store(reg_t addr, T val, xlate_flags_t xlate_flags = {}) {
    std::cout<<"mmu store is called\n";
    auto access_info = generate_access_info(addr, STORE, xlate_flags);
    reg_t paddr = translate(access_info, sizeof(T));
    std::cout<<"virtual adress is: "<<addr<<"\nphysical adress from store is: "<<paddr<<"\n";
    target_endian<T> target_val = to_target(val);
    bool flag = false;
    if(cache->get_name() == "regular cache" && cache->is_active())
    {
     flag = cache->store_from_proc(paddr, (const uint8_t*)&target_val, sizeof(T));
    }
    //std::cout<<"store from proc flag is: "<<flag<<"\n";
    if(!flag)
    {
     reg_t vpn = addr >> PGSHIFT;
     bool aligned = (addr & (sizeof(T) - 1)) == 0;
     ...
     if (!xlate_flags.is_special_access() && likely(aligned && tlb_hit)) {
       //std::cout<<"stored with tlb";
       *(target_endian<T>*)(tlb_data[vpn % TLB_ENTRIES].host_offset + addr) = target_val;//to_target(val);
       if(cache->get_name() == "regular cache" && cache->is_active())
       {
        std::cout<<"load to cache from store tlb is called\n";
        cache->load_to_cache(paddr, (uint8_t*) (tlb_data[vpn % TLB_ENTRIES].host_offset + addr));
        bool is_prefetched = cache->prefetch_on_miss(paddr, (uint8_t*) (tlb_data[vpn % TLB_ENTRIES].host_offset + addr));
       }
       //std::cout<<"data at host adress from tlb at mmu.h store is: ";
       //for (int i = 0;i<64;i++)
       //{
        //std::cout<<(reg_t) *(tlb_data[vpn%TLB_ENTRIES].host_offset + addr + i)<<" ";
       //}
       //std::cout<<"\n";
       //std::cout<<"data at host address tlb - 8 from store mmu.h is: ";
       //for (int i = 0;i<64;i++)
       //{
        //std::cout<<(reg_t) *(tlb_data[vpn%TLB_ENTRIES].host_offset + addr+i - 8)<<" ";
       //} 
       //std::cout<<"\n";
     } else {
       //target_endian<T> target_val = to_target(val);
       std::cout<<"store_slow_path is called\n";
