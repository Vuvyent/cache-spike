...
mmu_t::mmu_t(simif_t* sim, endianness_t endianness, processor_t* proc, cache_t* cache)
 : sim(sim), proc(proc), cache(cache),
...
void mmu_t::load_slow_path_intrapage(reg_t len, uint8_t* bytes, mem_access_info_t access_info)
{
  ...
  if (!access_info.flags.is_special_access() && vpn == (tlb_load_tag[vpn % TLB_ENTRIES] & ~TLB_CHECK_TRIGGERS)) {
    ...
    {
     cache->load_to_cache(paddr, (uint8_t*) host_addr);
     bool is_prefetched = cache->prefetch_on_miss(paddr, (uint8_t*) host_addr);
    }
    //std::cout<<"data at host adress from tlb from load_slow is: ";
    //for (int i = 0;i<64;i++)
    //{
    // std::cout<<(reg_t) *(host_addr+i)<<" ";
    //}
    //std::cout<<"\n";
    return;
  }
...
//paddr was here
  ...
  if (auto host_addr = sim->addr_to_mem(paddr)) {
    //std::cout<<"type of host_addr is: "<<typeid(host_addr).name()<<" host_addr from load_slow is: "<<(reg_t) host_addr<<"\n";
    memcpy(bytes, host_addr, len);
    if(cache->get_name() == "regular cache"&&cache->is_active())
    {
     cache->load_to_cache(paddr, (uint8_t*) host_addr);
    ...
    //std::cout<<"data at host adress from load is: ";
    //for (int i = 0;i<64;i++)
    //{
     //std::cout<<(reg_t) *(host_addr+i)<<" ";
    //}
...
void mmu_t::store_slow_path_intrapage(reg_t len, const uint8_t* bytes, mem_access_info_t access_info, bool actually_store)
{
  ...
  if (!access_info.flags.is_special_access() && vpn == (tlb_store_tag[vpn % TLB_ENTRIES] & ~TLB_CHECK_TRIGGERS)) {
    if (actually_store) {
      ...
      //std::cout<<"host_addr from tlb slow_store is: " <<(reg_t) host_addr<<"\n";
      //std::cout<<"data at host adress tlb from store_slow is: ";
      //for (int i = 0;i<64;i++)
      //{
      //std::cout<<(reg_t) *(host_addr+i)<<" ";
      //}
      //std::cout<<"\n";
      //std::cout<<"data at host address tlb - 8 from store_slow is: ";
      //for (int i = 0;i<64;i++)
      //{
      //std::cout<<(reg_t) *(host_addr+i - 8)<<" ";
      //} 
      //std::cout<<"\n";
    }
    return;
  ...
  //paddr was here
  //std::cout<<"virtual address from intrapage_store is: "<<addr<<"\nphysical address from intrapage_store is: "<< paddr<<"\n";

  if (actually_store) {
    if (auto host_addr = sim->addr_to_mem(paddr)) {
      //if(host_addr == NULL)
      //{
       //std::cout<<"host_addr is NULL";
      //}
      //else
      //{
      // std::cout<<" host_addr from store_slow is: "<<(reg_t) host_addr<<"\n";
      //}
      memcpy(host_addr, bytes, len);
      if(cache->get_name() == "regular cache" && cache->is_active())
      {
      ...
      //std::cout<<"data at host adress from store is: ";
      //for (int i = 0;i<64;i++)
      //{
       //std::cout<<(reg_t) *(host_addr+i)<<" ";
      //}
      //std::cout<<"\n";
      //std::cout<<"data at host address - 8 from store_slow is: ";
      //for (int i = 0;i<64;i++)
      //{
      // std::cout<<(reg_t) *(host_addr+i - 8)<<" ";
      //} 
